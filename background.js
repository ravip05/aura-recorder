...
// ---- Handlers ----
async function handleStartRecording(recSettings) {
    ...
    chrome.runtime.sendMessage(offscreenMsg).catch((err) => {
        console.warn('[Aura] Offscreen message failed:', err.message);
    });
}
async function handleStopRecording() {
    await chrome.storage.local.set({ isRecording: false });
    chrome.runtime.sendMessage({ action: 'STOP_RECORDING' }).catch((err) => {
...
async function handleRecordingComplete(blobId, duration) {
    await chrome.storage.local.set({ isRecording: false });
...
async function handleToggleCameraPreview(show, recSettings) {
    const { isRecording } = await chrome.storage.local.get('isRecording');
    if (isRecording)
        return; // Do not toggle preview if a recording is already active
    const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (activeTab?.id) {
        if (show) {
            await injectContentScript(activeTab.id);

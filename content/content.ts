...
(function() {
  if ((window as any).__AURA_INJECTED) return;
  (window as any).__AURA_INJECTED = true;
...
let videoWrap: HTMLDivElement | null = null;
...
function clampPosition(pos: { x: number; y: number }, elWidth: number, elHeight: number): { x: number; y: number } {
...
function buildDOM(cssText: string, settings: RecorderSettings, savedPositions: any = {}): void {
  ...
  const div = document.createElement('span'); div.className = 'ctrl-divider';
...
function makeCtrlBtn(extraClass: string, svgHTML: string, title: string): HTMLButtonElement {
...
async function startCamera(settings: RecorderSettings): Promise<void> {
...
function togglePause(btn: HTMLButtonElement, overlay: HTMLDivElement): void {
  isPaused = !isPaused;
...
function toggleMic(btn: HTMLButtonElement, indicator: HTMLDivElement): void {
...
function toggleShape(btn: HTMLButtonElement): void {
  isCircle = !isCircle;
...
function onDragStart(e: PointerEvent, el: HTMLElement): void {
  const target = e.target as HTMLElement;
  if (target.closest('.ctrl-btn') || target.closest('#aura-resize')) return;
...
async function showBubble(settings: RecorderSettings): Promise<void> {
